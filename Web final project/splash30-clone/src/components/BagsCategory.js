  ...
  useEffect(() => {
    ...
    const fetchProducts = async () => {
      try {
        ...
        // Filter products for the Bags category and limit to 5
        const filteredProducts = data
          .filter((product) => product.category === "Bags")
          .slice(0, 5); // Limit to 5 products
        setProducts(filteredProducts);
      } catch (error) {
        console.error("Error fetching bags category products:", error);
      }
    };
  ...
  const handleAddProduct = async () => {
    try {
      const response = await fetch("http://localhost:5000/api/products", {
        ...
        body: JSON.stringify({ ...form, category: "Bags" }),
      });
  ...
  return (
    <div className="bags-category">
      ...
      <div className="bags-header">
        <img
          src="https://splash30.com/cdn/shop/files/bag_1920x.jpg?v=1737462927"
          alt="Bags Category"
